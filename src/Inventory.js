...
function Sales() {
  ...
  const [didRemoveRow, setDidRemoveRow] = useState(false);
...
const handleReceivedChange = (e) => {
  ...
  // NEW VALIDATION WITH TOAST
  if (received > gross) {
    toast.error("Received amount cannot be greater than Gross Amount!", {
      position: "top-right",
      autoClose: 2000,
...
const handleRemoveRow = () => {
  ...
  if (updatedRowData.length === 0) {
    setRowData([
      { serialNumber: 1, Code: '', ServiceID: '', ServiceName: '', Rate: null },
    ]);
    ...
    setNet_Amount(0);
    setDiscount(0);
    // don't set balance here — use effect to ensure no overwrites
  } else {
    setRowData(updatedRowData);
  }

  // signal that we removed a row (component effect will set balance to 0)
  setDidRemoveRow(true);
};

// effect that responds to the removal and forces balance to zero
useEffect(() => {
  if (!didRemoveRow) return;

  // Force balance to zero
  setBalance_Amount(0);

  // clear flag immediately so next operations are unaffected
  setDidRemoveRow(false);
}, [didRemoveRow]);
                ...
                <div className="exp-form-floating">
                  ...
                  <input
                    ...
                    autoComplete="off"
                       onChange={(e) => {
        const value = e.target.value;
        if (/^\d*$/.test(value)) { // only numbers (0–9)
          setAge(value);
        }
      }}
                  />
                ...
                <div className="col-md-2 form-group mb-2">
                  ...
                  <input
                    id="discount"
                     ...
                     value={Discount}
                     onChange={(e) => {
                       const cleanedValue = e.target.value.replace(/[^0-9.]/g, ""); // allow only numbers & dot
