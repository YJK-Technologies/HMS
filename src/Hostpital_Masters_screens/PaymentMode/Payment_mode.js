 ...
 const [Address, setAddress] = useState("");
          ...
          ClientID,
           ClientName,
            ContactPerson,
             ...
             AgreementType,
             Address,
            company_code : company_code }), // Send as search criteria
      });
  ...
  const onCellValueChanged = (params) => {
    ...
    const rowIndex = updatedRowData.findIndex(
      (row) => row.ClientID === params.data.ClientID && row.ClientName === params.data.ClientName
    );
    if (rowIndex !== -1) {
...
const saveEditedData = async () => {
  ...
  const company_code = sessionStorage.getItem('selectedCompanyCode');
  const formattedModifiedDate = formatDate(new Date()); // Use current date for modified_date

  // Filter only rows that were edited & selected
  const selectedRowsData = editedData.filter(row =>
    selectedRows.some(selectedRow =>
      selectedRow.ClientID === row.ClientID && selectedRow.ClientName === row.ClientName
    )
  );
  ...
  // Map the data to match backend's expected fields
  const payload = {
    editedData: selectedRowsData.map(row => ({
  ...
  showConfirmationToast(
    ...
    async () => {
      setLoading(true);
      try {
        const response = await fetch(`${config.apiBaseUrl}/ClientMasterUpdate2`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
  ...
  const deleteSelectedRows = async () => {
    ...
    const modified_by = sessionStorage.getItem('selectedUserCode');
    const company_code = sessionStorage.getItem('selectedCompanyCode');
    const ClientID = selectedRows.map((row) => row.ClientID);
    const ClientName = selectedRows.map((row) => row.ClientName);
    ...
    showConfirmationToast(
      ...
      async () => {
        ...
        try {
          const response = await fetch(`${config.apiBaseUrl}/ClientMasterDelete`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "company_code": company_code,
            ...
            body: JSON.stringify({ ClientID, ClientName, company_code }),
             "company_code": company_code,
            "modified_by": modified_by
      ...
      <div>
        ...
        <div className="shadow-lg p-1 bg-body-tertiary rounded  mb-2 mt-2">
          <div className="row ms-4 mt-3 mb-3 me-4">
            ...
            <label className="form-label fw-semibold d-flex justify-content-start">Status </label>
            <Select
                            id="AgreementType"
